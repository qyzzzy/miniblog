...
import (
	...
	"github.com/qylearn/miniblog/pkg/version/verflag"
)
...
func run() error {
	...
	if err := httpsrv.Shutdown(ctx); err != nil {
		log.Errorw("Insecure Server force to shutdown", "err", err)
		return err
	}
...
func startSecureServer(g *gin.Engine) *http.Server {
	// 创建HTTP Server
	httpsrv := &http.Server{Addr: viper.GetString("tls.addr"), Handler: g}
	// 运行HTTP服务器
	// 打印一条日志，用来提示HTTP服务器已经起来，方便排修
	log.Infow("Start to listening the incoming requests on http address", "addr", viper.GetString("addr"))
...
func startGRPCServer() *grpc.Server {
	lis, err := net.Listen("tcp", viper.GetString("gprc.addr"))
	if err != nil {

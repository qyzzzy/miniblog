...
func (s *ServerOptions) AddFlags(flag *pflag.FlagSet) {
	flag.StringVar(&s.ServerMode, "server-mode", s.ServerMode, fmt.Sprintf("Server mode, available options: %v", availableServerModes.UnsortedList()))
	flag.StringVar(&s.JWTKey, "jwt-key", s.JWTKey, "JWT signing key. Must be at least 6 characters long.")
	flag.DurationVar(&s.Expiration, "expiration", s.Expiration, "The expiration duration of JWT tokens.")
}
